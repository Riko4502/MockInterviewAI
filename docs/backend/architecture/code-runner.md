...
Сервис не имеет собственной БД и состояния: он stateless-прослойка между внутренними потребителями (`apps/api`, `apps/realtime`) и Judge0.
...
Сервис внутренний: наружу не публикуется, вызывается из `apps/api` и `apps/realtime` по адресу `http://code-runner:8090`.
