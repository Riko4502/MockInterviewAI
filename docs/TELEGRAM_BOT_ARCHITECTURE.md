...
1. **Автоопределение**: на основе `ctx.from.language_code` из Telegram API.
2. **Язык профиля**: если аккаунт привязан, используется сохраненная локаль пользователя.
3. **Ручной выбор**: команда `/lang` позволяет принудительно переключить язык (сохраняется в сессии/кэше бота).
