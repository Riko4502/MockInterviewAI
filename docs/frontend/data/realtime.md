...
> **Практика работы с SSE на фронте** (подключение через `fetch`, реконнект, Replay по `Last-Event-ID`, обработка ошибок, тесты) — в отдельном руководстве: [sse.md](./sse.md). Ниже — справочник типов и контрактов.
...
```
[ Frontend (Client) ]                             [ API (ticket) ]          [ Realtime Server ]                        [ LiveKit SFU ]
          |                                                |                          |                                        |
          ...
          |                                                |                          |---- 11. Бродкаст "code.update" ------->|
          |---------------- 12. event: "cursor.move" (line: 12) ---------------------->>|                                        |
          |                                                |                          |---- 13. Бродкаст "cursor.move" ------->|
          |                                                |                          |                                        |
          |---------------- 14. event: "ai.request_hint" ----------------------------->|                                        |
          |                                                |                          |------ (Вызов LLM воркера)               |
          |<--------------- 15. event: "ai.suggestion" ---------------------------------|                                        |
```
