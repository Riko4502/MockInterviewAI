...
| 3 | **Security / Features** | Сброс пароля пользователя администратором с генерацией временного пароля | Невозможность восстановить доступ скомпрометированному пользователю или пользователю без доступа к email | Добавить эндпоинт `POST /api/v1/admin/users/:id/reset-password` (генерация временного пароля сервером, логирование в консоль до появления email-сервиса, инкремент `generation` и отзыв сессий) | 🟡 Средний | ✅ Выполнено |
...
| 6 | **Performance / DB** | Неоптимальный поиск по подстроке (`ILIKE '%query%'`) без Trigram-индекса | Full Table Scan (Seq Scan) при большом количестве пользователей в продакшене | Добавить миграцию PostgreSQL с расширением `pg_trgm` и триграммными GIN-индексами для `email`, `username`, `displayName` | 🟢 Низкий | ⏳ Ожидает |
...
2. **Безопасность сброса пароля администратором (Zero-Knowledge & Email Delivery):**
   ...
   - *Решение:*
     ...
     - Пароль отправляется пользователю на email. Пока в монорепозитории отсутствует почтовый сервис, сгенерированный пароль **логируется в консоль** (`this.logger.log(...)`).
...
- [x] **3.2.** В `AdminUsersService`:
  ...
  - Залогировать сгенерированный временный пароль в консоль: `this.logger.log(\`[TEMP PASSWORD] Reset password for user \${user.email} (\${id}): \${tempPassword}\`);` (до внедрения email-сервиса).
...
- [ ] **6.1.** Создать SQL-миграцию:
  ...
  ```
- [ ] **6.2.** Верифицировать план выполнения через `EXPLAIN ANALYZE`.
