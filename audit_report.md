...
| -------- | -------- | ------- | -------- | --- |
| P0 | Security / Race Condition | Возможность обхода проверки `generation` (CWE-362/613) из-за слабого тестирования и обработки ошибок | `apps/api/src/common/guards/access-token.guard.ts` | CWE-362 / CWE-613 |
...
1. **Гонки в `access-token.guard.ts` (CWE-362/613)**: Хотя guard содержит логику `fail-closed`, тесты вида `rejects.toThrow("redis down")` показывают, что в случае падения Redis защита просто блочит всех (500). Если база PostgreSQL также будет временно недоступна, защита может вести себя непредсказуемо, так как тесты проверяют только проброс исключений, а не их изоляцию (например, отсутствие фоллбэка на кеш).
2. **False-positive тесты (`auth.service.spec.ts`)**: Использование `.catch(() => undefined)` внутри тестов при проверке логирования (например, "raw токены не попадают в логи"). Если тестируемый метод завершится успешно (вместо ожидаемого сброса с ошибкой), `catch` ничего не поймает, логи не будут записаны, и `expect(logged).not.toContain(...)` успешно пройдет! Это маскирует отсутствие генерации ошибки.
3. **Молчаливое подавление ошибок удаления AuthRevocationTask (`auth.service.ts`, строки 447, 547, 827)**: Ошибки при удалении задач из базы глушатся. Если БД недоступна, задача останется висеть, что вызовет бесконечный цикл повторов.
