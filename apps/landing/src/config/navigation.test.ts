...
describe("navigationConfig & Auth Links", () => {
  ...
  it("should generate valid register URL", () => {
    ...
    expect(
      registerUrl.startsWith("http://") ||
        registerUrl.startsWith("https://") ||
        registerUrl.startsWith("/"),
    ).toBe(true);
  });
  ...
  it("should clean trailing slashes properly without creating double slashes", () => {
    ...
    expect(getRegisterUrl(dirtyUrl)).toBe(
      "https://auth.mockinterview.com/register",
    );
  });

  it("should construct application sub-paths correctly", () => {
    expect(getAppUrl("/dashboard", "https://app.custom.com")).toBe(
      "https://app.custom.com/dashboard",
    );
    expect(getAppUrl("dashboard", "https://app.custom.com")).toBe(
      "https://app.custom.com/dashboard",
    );
    expect(getAppUrl("/tracks/frontend", "https://app.custom.com")).toBe(
      "https://app.custom.com/tracks/frontend",
    );
  });

  it("should have valid repository link", () => {
    expect(navigationConfig.githubUrl).toBe(
