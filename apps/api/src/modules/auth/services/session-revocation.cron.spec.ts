...
describe("SessionRevocationCron", () => {
  ...
  describe("handleCron", () => {
    ...
    it("успешно обрабатывает пачку задач, отзывает сессии и удаляет задачи", async () => {
      const mockTasks = [
        { id: "task-1", userId: "user-1", createdAt: new Date() },
        { id: "task-2", userId: "user-2", createdAt: new Date() },
      ];
    ...
    it("при ошибке обработки одной задачи продолжает обработку остальных и оставляет упавшую", async () => {
      const mockTasks = [
