...
describe("getErrorMessage", () => {
  ...
  it("форматирует ошибки валидации, если error.data сам является объектом полей", () => {
    const error = {
      message: "HTTP 400",
      data: {
        newPassword: "Пароль должен содержать минимум 12 символов",
        newPasswordConfirmation: "Пароли не совпадают",
      },
    };

    expect(getErrorMessage(error, "Запасное сообщение")).toBe(
      "Пароль должен содержать минимум 12 символов. Пароли не совпадают",
    );
  });
});
