/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Mock Interview AI API
 * Auth API: register, login, logout.
 *
 * Refresh token передаётся только через HttpOnly cookie
 * (HttpOnly; SameSite=Lax; Path=/api/v1/auth; Max-Age=JWT_REFRESH_EXPIRATION)
 * и не возвращается в JSON response.
 * OpenAPI spec version: 0.1.0
 */

/**
 * Роль пользователя в сессии
 */
export type JoinSessionResponseDtoRole = typeof JoinSessionResponseDtoRole[keyof typeof JoinSessionResponseDtoRole];


export const JoinSessionResponseDtoRole = {
  CANDIDATE: 'CANDIDATE',
  INTERVIEWER: 'INTERVIEWER',
  OBSERVER: 'OBSERVER',
} as const;
